...
def parse_ocr_results(ocr_results):
    ...
    for text in ocr_results:
        ...
        # 🔹 "/숫자km" 또는 "/숫자.숫자km" 패턴 감지 후 슬래시 제거
        match = re.search(r"/(\d+[\.,]?\d*km)", text)  # '/'를 제외한 숫자.km 추출
        if match:
            distance = match.group(1)  # 정규식 그룹에서 '/' 없는 값 추출
            continue  # 이후 처리 로직을 건너뛰고 다음 루프 진행
        ...
        if "월" in text and len(text) <= 4:  # ex: "8월"
            ...
            if len(split_values) == 2:
                ...
                if distance is None:  # 이미 설정된 값이 없다면 설정
